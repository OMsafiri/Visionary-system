...
if(isset($_POST['submit'])){
	...
	if(isset($_SERVER['REQUEST_METHOD']) == 'POST'){
		...
		$query = "UPDATE tbl_contributions SET member_id=?,category_id=?,amount=?,balance=? WHERE id = $user_id";
		$stmt = $instance->prepare($query);
		$stmt->bind_param("ssss",$member_id, $category_id, $amount, $current_balance);
		$stmt->execute();

		if ($stmt->affected_rows > 0) {
			header("location:accountant_member_contributions.php");
		}else {
			print_r($stmt);
		}
	}
					...
					<div class="row">
						...
						<div class="col-md-12">
							<form method="POST" action="" autocomplete="off">
								...
								<div class="row">
									<div class="col-md-12">
										<button name="submit" type="submit" class="btn btn-lg btn-primary">Update Contribution</button>
									</div>
								</div>
							</form>
						</div>
