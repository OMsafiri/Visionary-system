...
if(isset($_POST['submit'])){
	...
	if(isset($_SERVER['REQUEST_METHOD']) == 'POST'){
		...
		$current_member = get_contribution_by_user($instance,$member_id, $category_id);
		...
		$query = "INSERT INTO tbl_contributions(user_id,category_id,amount, balance) VALUES(?,?,?,?)";
		$stmt = $instance->prepare($query);
		$stmt->bind_param("ssss", $member_id, $category_id, $amount, $current_balance);
		$stmt->execute();

		if ($stmt->affected_rows > 0) {
			echo"<script>window.location.href='member_contributions.php';</script>";
		}else {
			print_r($stmt);
		}
	}
								...
								<div class="row">
									...
									<div class="col-3">
										<div class="mb-3">
											...
											<select class="form-control" name="category_id" id="inputCategory" disabled>
